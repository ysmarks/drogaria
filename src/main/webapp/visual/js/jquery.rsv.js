...
(function($) {
  ...
  $.fn.RSV.defaults = {
    ...
    errorFieldClass:      "pv-campo-erro",
    errorTextIntro:       "Foram encontrados problemas no preenchimento de alguns campos:",
    errorJSItemBullet:    "* ",
    errorHTMLItemBullet:  "",
    errorTargetElementId: "pv-erros-validacao",
    ...
    for (var i=0; i<rules.length; i++)
    {
      ...
      if (requirement != "function" && options.errorFieldClass)
      {
        if (form[fieldName].type == undefined)
        {
          ...
          {
			if($(form[fieldName][j]).parent().hasClass('pv-campo-fluido')){
				if ($(form[fieldName][j]).parent('div').hasClass(options.errorFieldClass))
					$(form[fieldName][j]).parent('div').removeClass(options.errorFieldClass);
			}
            else{
				if ($(form[fieldName][j]).parents('div[class^="pv-col"]').hasClass(options.errorFieldClass))
					$(form[fieldName][j]).parents('div[class^="pv-col"]').removeClass(options.errorFieldClass);				
			}
          }
        }
        else
        {
			if($(form[fieldName]).parent().hasClass('pv-campo-fluido')){
				if ($(form[fieldName]).parent('div').hasClass(options.errorFieldClass))
					$(form[fieldName]).parent('div').removeClass(options.errorFieldClass);
			}
			else{
				if ($(form[fieldName]).parents('div[class^="pv-col"]').hasClass(options.errorFieldClass))
					$(form[fieldName]).parents('div[class^="pv-col"]').removeClass(options.errorFieldClass);
			}
        }
      }
      ...
      if(!aplicaValidacao(requirement, form, fieldName, errorMessage)){
         return false;
      }
    }
  ...
  function styleField(field, focus)
  {
    ...
    if (field.type == undefined)
    {
      ...
      {
		if($(field[i]).parent().hasClass('pv-campo-fluido')){
			if (!$(field[i]).parent('div').hasClass(options.errorFieldClass))
			  $(field[i]).parent('div').addClass(options.errorFieldClass);
		}
		else{
			if (!$(field[i]).parents('div[class^="pv-col"]').hasClass(options.errorFieldClass))
			  $(field[i]).parents('div[class^="pv-col"]').addClass(options.errorFieldClass);			
		}
    ...
    else
    {
      if (options.errorFieldClass)
		if($(field).parent().hasClass('pv-campo-fluido')){
			$(field).parent('div').addClass(options.errorFieldClass);
		}
		else{
			$(field).parents('div[class^="pv-col"]').addClass(options.errorFieldClass);			
		}
      if (focus){
	  	try{
			field.focus();
  ...
  function aplicaValidacao(requirement, form, fieldName, errorMessage){
     // now, validate whatever is required of the field
      switch (requirement)
      {
        case "required":
          ...
          else if (!$.trim(removeTabulacao(form[fieldName].value)))
          {
            if (!processError(form[fieldName], errorMessage))
              return false;
          }
        ...
        default:
          var aux = requirement.split("=");
          //alert('var result = '+aux[0]+'("'+form[fieldName].value+'"); - '+requirement.replace(aux[0]+"=",""));
          eval('var result = '+aux[0]+'("'+form[fieldName].value+'");');
          if(aux[1]==undefined){
             if (!result)
             {   if (!processError(form[fieldName], errorMessage)) return false;
             }
          }
          else{
             if (result)
             {  
            	 if(!aplicaValidacao(requirement.replace(aux[0]+"=",""), form, fieldName, errorMessage)){
                     if (!processError(form[fieldName], errorMessage))
                         return false;
                 }
             }
